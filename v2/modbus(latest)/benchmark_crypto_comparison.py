...
print(
    f"Total Payload Size  : {total_payload_bytes:,} bytes ({total_payload_bytes / 1024:.2f} KB)")
...
def run_xor16():
    ...
    key_stream = np.random.randint(
        0, 65536, size=raw_uint16.shape, dtype=np.uint16)
...
def run_xor64():
    ...
    key_64 = rng.integers(0, np.iinfo(np.uint64).max, size=len(
        raw_64), dtype=np.uint64, endpoint=True)
...
def run_chacha20():
    ...
    dec_uint16 = np.frombuffer(
        decrypted_bytes, dtype=np.uint16).reshape(raw_uint16.shape)
    max_err = np.abs(raw_uint16.astype(int) - dec_uint16.astype(int)).max()
    exact_pct = (raw_uint16 == dec_uint16).mean() * 100
...
def run_aes_ctr():
    ...
    t1 = time.perf_counter()
    cipher_dec = Cipher(algorithms.AES(key), modes.CTR(
        nonce), backend=default_backend())
...
for name, func in methods:
    ...
    throughput_mb_sec = (total_payload_bytes /
                         (1024 * 1024)) / (np.mean(enc_times))

    size_overhead_pct = ((c_bytes - total_payload_bytes) /
    ...
    print(
        f"Completed {name:26}: Enc = {avg_enc_ms:7.3f} ms ({latency_enc_us:8.4f} µs/sample) | Match = {exact_pct:.1f}%")
...
sample_uint64 = (sample_uint16[0].astype(np.uint64) << 48) | (sample_uint16[1].astype(
    np.uint64) << 32) | (sample_uint16[2].astype(np.uint64) << 16) | sample_uint16[3].astype(np.uint64)
...
t0 = time.perf_counter()
for _ in range(N_PACKETS):
    c = Cipher(algorithms.ChaCha20(b'0123456789abcdef0123456789abcdef',
               b'0123456789abcdef'), mode=None, backend=default_backend())
    enc = c.encryptor()
...
t0 = time.perf_counter()
for _ in range(N_PACKETS):
    c = Cipher(algorithms.AES(b'0123456789abcdef'), modes.CTR(
        b'0123456789abcdef'), backend=default_backend())
    enc = c.encryptor()
...
for _ in range(N_PACKETS):
    ...
    c = Cipher(algorithms.AES(b'0123456789abcdef'), modes.CBC(
        b'0123456789abcdef'), backend=default_backend())
    enc = c.encryptor()
...
print(
    f"  XOR-64 (SIMD)           : {t_pkt_xor64:9.4f} µs/packet  (Baseline: 1.0x)")
print(
    f"  XOR-16 (Register-level) : {t_pkt_xor16:9.4f} µs/packet  ({t_pkt_xor16/t_pkt_xor64:.1f}x vs XOR-64)")
print(
    f"  ChaCha20 (CSPRNG Stream): {t_pkt_chacha:9.4f} µs/packet  ({t_pkt_chacha/t_pkt_xor64:.1f}x slower)")
print(
    f"  AES-128-CTR (Stream)    : {t_pkt_aes_ctr:9.4f} µs/packet  ({t_pkt_aes_ctr/t_pkt_xor64:.1f}x slower)")
print(
    f"  AES-128-CBC (Block+Pad) : {t_pkt_aes_cbc:9.4f} µs/packet  ({t_pkt_aes_cbc/t_pkt_xor64:.1f}x slower)")
...
for row in summary_rows:
    row['Per_Packet_Latency_us'] = round(
